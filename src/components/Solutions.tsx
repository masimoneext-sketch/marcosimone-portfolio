...
const solutions: Solution[] = [
  ...
  },
  {
    ...
    problem: "Gestite ancora l'inventario IT su fogli Excel legacy?",
    solution:
      "Portale Laravel 11 + React per il lifecycle di migliaia di asset IT (PC, server, periferiche): import massivo da XLS legacy (~6.400 asset già testati su 14 colonne), documenti allegati, storico interventi, auth JWT custom a 3 ruoli. Deploy flessibile: Docker su Linux oppure IIS + PHP-CGI su Windows Server esistente.",
    targetAudience: 'IT Manager di aziende e università con 100+ device da gestire',
    roi: 'Visibilità totale sul parco asset — import in un colpo dallo spreadsheet esistente',
    tags: ['Asset Tracking', 'ITSM', 'Legacy Import', 'Compliance'],
    color: 'purple',
  },
  {
    icon: '📚',
  ...
  },
  {
    ...
    problem: 'Chi ha preso quale laptop, quando lo riporta e quanto lo usa davvero?',
    solution:
      "Portale Node.js + SQLite per gestire prestiti di device aziendali con tracciabilità completa, ranking di utilizzo per laptop, import massivo da CSV, export per inventario, statistiche e audit log. Auth JWT a 3 ruoli (user/admin/superadmin), processo gestito via PM2 dietro reverse proxy Docker + Traefik con TLS automatico.",
    targetAudience: 'Uffici IT con pool hardware condiviso',
    roi: 'Zero dispositivi persi — tracciabilità al 100% + dati reali di utilizzo per rinnovo parco',
    tags: ['Hardware Tracking', 'IT Operations', 'Audit Log'],
    color: 'purple',
  },
  {
    icon: '📅',
  ...
  },
  {
    ...
    problem: 'I tuoi agenti AI sprecano token, toccano file fuori scope e non rispettano il preventivo?',
    solution:
      "Skill per Claude Code con Metodo Capocantiere v4: Triage hard di Fase 0.5 che può abortire se il task non qualifica, costruzione a strati (fondamenta → struttura → mura → finiture) con gate d'ispezione ad ogni livello, ed enforcement fisico tramite PreToolUse hook (blocca la scrittura fuori dall'allowlist) e validator Python standalone che grep-verifica i contratti. Opus progetta, Sonnet costruisce — mai invertire.",
